...
def check_np_equivalence(fun_name, args, kwargs, array_argnums):
    ans_truth = getattr(onp, fun_name)(*args, **kwargs) # Original numpy, regular args
    gnp_fun = getattr(gnp, fun_name)
    for dtypes in it.product(*[[gnp.float64, gnp.gpu_float32]]*len(array_argnums)):
        ...
        assert type(ans_truth) is type(ans_test), \
            "Type mismatch! \nTruth:  {0}, \nResult: {1}.\ndtypes were {2}, args were {3}".format(
                type(ans_truth), type(ans_test), dtypes, args)
        assert onp.allclose(ans_truth, ans_test, atol=1e-5), \
            "Value mismatch.\nTruth:  {0}, \nResult: {1}. \ndtypes were {2}, args were {3}".format(
                ans_truth, ans_test, dtypes, args)
...
def unary_ufunc_check(fun_name, lims=[-2, 2]):
    ...
    if test_complex:
        ...
        combo_check(fun_name, (0,), [comp, matc])
